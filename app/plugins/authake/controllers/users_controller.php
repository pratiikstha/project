...
class UsersController extends AuthakeAppController {
  var $name     = 'Users';
  var $uses     = array('Authake.User', 'Authake.Permission');
  var $components = array('Authake.Filter',  'Authake.Authake');
  //var $layout = 'authake';
  ...
  //var $scaffold;
    ...
    function index($tableonly = false) {
        ...
        $this->set('currentgroups', $this->Authake->getGroupIds());
    }
    ...
    function add() {
        if (!empty($this->data)) {
			...
			// only an admin can make an admin
			if (!in_array(1, $this->data['Group']['Group'])&& !array_intersect($this->data['Group']['Group'], $this->Authake->getGroupIds())) {
				$this->Session->setFlash(__('You cannot add a user in other group unless you are admin', true), 'warning');
				$this->redirect(array('action'=>'index'));
			}
